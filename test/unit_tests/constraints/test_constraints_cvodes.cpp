...
int main(int argc, char* argv[])
{
  ...
  int flag = CVodeInit(cvode_mem, ode_rhs, SUN_RCONST(0.0), y);
  ...
  if (check_flag(flag, "CVodeSetConstraints")) { return 1; }
  ...
  flag = CVodeSetInitStep(cvode_mem, init_step);
  if (check_flag(flag, "CVodeSetInitStep")) { return 1; }
  ...
  sunrealtype h_init;
  flag = CVodeGetActualInitStep(cvode_mem, &h_init);
  if (check_flag(flag, "CVodeGetActualInitStep")) { return 1; }
