...
int main(int argc, char* argv[])
{
  ...
  /* Create the shared SUNDIALS context used by IDA, the N_Vectors, and the
  ...
  /* First check the default path. IDA should not create an internal vector
  ...
  /* Free the first IDA memory block. Since IDA created the stack above,
     IDAFree is responsible for destroying it. */
  IDAFree(&ida_mem);

  /* Next check the user-supplied path. This new IDA memory block will receive
  ...
  /* Seed the user-owned stack with one vector so we can verify after IDAFree
     that the stack still exists and has not been destroyed by IDA. */
  flag = SUNVecStack_Create(y, 1, sunctx, &stack);
  if (check_flag("SUNVecStack_Create", flag)) { return 1; }

  /* Attach the user-owned stack. IDA should store this pointer but leave
  ...
  /* The checked-out IDA workspace vectors should now be active in the
  ...
  /* Attempting to replace the stack while IDA has checked-out vectors should
  ...
  /* IDAFree must not destroy a user-owned stack. The caller should still be
     able to query and destroy it after the IDA memory is gone. */
  IDAFree(&ida_mem);
