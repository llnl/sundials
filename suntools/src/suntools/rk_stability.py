...
``stability_magnitude(p, q, Z)``  |phi(Z)|.
...
def stability_magnitude(p, q, Z):
    """|phi(Z)| = |p(Z) / q(Z)| for scalar or array Z, with divide/invalid warnings suppressed.

    Suppressing the warnings keeps batch plotting quiet when poles are present; the values
    (including inf/nan) are preserved for contouring.
    """
    with np.errstate(divide="ignore", invalid="ignore"):
        return np.abs(p(Z) / q(Z))
...
def max_axis_crossing(phi: StabilityFunction) -> float:
    ...
    for axis in ("real", "imag"):
        ...
        if roots.size == 0:
            continue
        magnitude = stability_magnitude(phi.p, phi.q, boundary.point(roots))
