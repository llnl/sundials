...
def _poly_str(poly: np.poly1d, var: str = "z") -> str:
    return str(np.poly1d(poly.c, variable=var)).strip()
...
def describe(table: ButcherTable, show_embedding: bool = True) -> None:
    """Print the tableau and stability function(s) for one method."""
    print(table)
    print()
    phi = table.stability_function
    p, q = phi.p, phi.q
    print("Stability function  phi(z) = p(z) / q(z):")
    print("  numerator p(z):")
    print(_indent(_poly_str(p)))
    if q.order > 0:
        print("  denominator q(z):")
        print(_indent(_poly_str(q)))
    else:
        print("  denominator q(z): 1   (explicit method -> phi is a polynomial)")

    phi = table.embedded_stability_function if show_embedding else None
    if phi is not None:
        order = f", order {table.embedding_order}" if table.embedding_order is not None else ""
        print(f"\nEmbedded method{order}: phi_hat(z) = p_hat(z) / q(z) (same denominator)")
        print("  numerator p_hat(z):")
        print(_indent(_poly_str(phi.p)))
    elif table.has_embedding:
        print("\n(embedded method present but its region plot is disabled)")
    print()
...
def plot_one(tables: dict[str, ButcherTable], method: str, outfile: str | None,
    ...
    table = tables[method]
    describe(table, show_embedding=options.show_embedding)
...
def plot_all(tables: dict[str, ButcherTable], source: str, outdir: str,
             options: PlotOptions) -> None:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        "--suppress-islands",
        dest="suppress_pinholes",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="hide tiny disconnected stable islands around zeros of phi "
        "(they render as stray dots)",
    )
    ap.add_argument(
...
def main(argv=None):
    ...
    source = _source_label(args.deffile)
    if args.list:
