...
6. If `gh` is available, optionally post reply comments back to the PR review threads.

   Use this when the user wants the branch updates reflected on GitHub as well. For
   each addressed inline review comment, post a short reply that:

   - summarizes what was changed,
   - stays specific to that thread,
   - explicitly states that the message was generated by an agent.

   Prefer a format like:

   ```text
   Agent-generated update: addressed this by <short summary>. Verified with <short verification>.
   ```

   Use `gh api` against the review-comment replies endpoint:

   ```bash
   gh api \
     repos/<owner>/<repo>/pulls/comments/<comment_id>/replies \
     -f body='Agent-generated update: addressed this by <summary>.'
   ```

   Guardrails:

   - Always ask the user for permission to do this.
   - Only reply to comments that were actually addressed locally.
   - Do not claim verification you did not run.
   - If several comment IDs were resolved by one shared change, tailor each reply to that thread instead of posting the same generic message everywhere.
