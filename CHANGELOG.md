...
We added the function `ARKodeAllocateInternalData` to ARKODE to enable
stage-related data allocation before the first call to `ARKodeEvolve` (but after
all other optional input routines have been called), to support users who measure
memory usage before beginning a simulation.
We added the function `ARKodeGetStageIndex` that returns the index of the stage
