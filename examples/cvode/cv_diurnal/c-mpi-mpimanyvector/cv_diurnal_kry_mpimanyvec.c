 ...
 * -----------------------------------------------------------------
 * Example problem (based on cv_diurnal_kry_p.c):
 *
 ...
 *
 * Execution: mpirun -np N cv_diurnal_kry_mpimanyvec
...
typedef struct
{
  ...
  sunrealtype c1ext[(MXSUB + 2) * (MYSUB + 2)];
  sunrealtype c2ext[(MXSUB + 2) * (MYSUB + 2)];
  sunrealtype SendBufferE[NVARS * MYSUB];
  sunrealtype SendBufferW[NVARS * MYSUB];
  sunrealtype SendBufferN[NVARS * MXSUB];
  sunrealtype SendBufferS[NVARS * MXSUB];
  sunrealtype RecvBufferE[NVARS * MYSUB];
  sunrealtype RecvBufferW[NVARS * MYSUB];
  sunrealtype RecvBufferN[NVARS * MXSUB];
  sunrealtype RecvBufferS[NVARS * MXSUB];
...
static void BSend(sunrealtype c1data[], sunrealtype c2data[], UserData data);
static void BRecvPost(UserData data);
static void BRecvWait(UserData data);
static void PrepareExt(N_Vector u, UserData data);
static void fcalc(sunrealtype t, N_Vector udot, UserData data);
...
int main(int argc, char* argv[])
{
  ...
  if (check_retval((void*)c[0], "N_VNew_Parallel", 0, my_pe))
  {
    MPI_Abort(comm, 1);
  }
  c[1] = N_VNew_Parallel(comm, local_N, neq, sunctx);
  if (check_retval((void*)c[1], "N_VNew_Parallel", 0, my_pe))
  {
    MPI_Abort(comm, 1);
  }
  u = N_VNew_MPIManyVector(2, c, sunctx);
  if (check_retval((void*)u, "N_VNew_MPIManyVector", 0, my_pe))
  {
    MPI_Abort(comm, 1);
  }
...
static void SetInitialProfiles(N_Vector u, UserData data)
{
  int lx, ly, jx, jy;
  sunindextype offset;
  sunrealtype x, y, cx, cy, xmid, ymid;
  sunrealtype *c1data, *c2data;
...
{
  int qu, retval;
  sunrealtype hu, *c1data, *c2data, tempu[2];
  int npelast;
  long int nst;
  MPI_Status status;
...
static void BRecvPost(UserData data)
{
  ...
  if (data->isuby > 0)
  {
    ...
    if (check_retval(&retval, "MPI_Irecv", 3, data->my_pe))
    {
      MPI_Abort(data->comm, 1);
    }
  }
...
static void BSend(sunrealtype c1data[], sunrealtype c2data[], UserData data)
{
  ...
  if (data->isuby < NPEY - 1)
  {
    ...
    for (lx = 0; lx < MXSUB; lx++)
    {
      data->SendBufferN[lx] = c1data[offsetu + lx];
    }
    for (lx = 0; lx < MXSUB; lx++)
    {
      data->SendBufferN[MXSUB + lx] = c2data[offsetu + lx];
    }
  ...
  if (data->isubx > 0)
  {
    for (ly = 0; ly < MYSUB; ly++)
    {
      data->SendBufferW[ly] = c1data[ly * MXSUB];
    }
    for (ly = 0; ly < MYSUB; ly++)
    {
      data->SendBufferW[MYSUB + ly] = c2data[ly * MXSUB];
    }
  ...
  if (data->isubx < NPEX - 1)
  {
    for (ly = 0; ly < MYSUB; ly++)
    {
      data->SendBufferE[ly] = c1data[(ly + 1) * MXSUB - 1];
    }
    for (ly = 0; ly < MYSUB; ly++)
    {
      data->SendBufferE[MYSUB + ly] = c2data[(ly + 1) * MXSUB - 1];
    }
...
static void BRecvWait(UserData data)
{
  ...
  if (data->isuby > 0)
  {
    ...
    if (check_retval(&retval, "MPI_Wait", 3, data->my_pe))
    {
      MPI_Abort(data->comm, 1);
    }
    ...
    for (lx = 0; lx < MXSUB; lx++)
    {
      data->c1ext[1 + lx] = data->RecvBufferS[lx];
    }
    for (lx = 0; lx < MXSUB; lx++)
    {
      data->c2ext[1 + lx] = data->RecvBufferS[MXSUB + lx];
    }
  }
  ...
  if (data->isuby < NPEY - 1)
  {
    ...
    for (lx = 0; lx < MXSUB; lx++)
    {
      data->c1ext[(MYSUB + 1) * (MXSUB + 2) + 1 + lx] = data->RecvBufferN[lx];
    }
    for (lx = 0; lx < MXSUB; lx++)
    {
      data->c2ext[(MYSUB + 1) * (MXSUB + 2) + 1 + lx] =
        data->RecvBufferN[MXSUB + lx];
    }
  }
  ...
  if (data->isubx > 0)
  {
    ...
    for (ly = 0; ly < MYSUB; ly++)
    {
      data->c1ext[(ly + 1) * (MXSUB + 2)] = data->RecvBufferW[ly];
    }
    for (ly = 0; ly < MYSUB; ly++)
    {
      data->c2ext[(ly + 1) * (MXSUB + 2)] = data->RecvBufferW[MYSUB + ly];
    }
  }
  ...
  if (data->isubx < NPEX - 1)
  {
    ...
    for (ly = 0; ly < MYSUB; ly++)
    {
      data->c1ext[(ly + 2) * (MXSUB + 2) - 1] = data->RecvBufferE[ly];
    }
    for (ly = 0; ly < MYSUB; ly++)
    {
      data->c2ext[(ly + 2) * (MXSUB + 2) - 1] = data->RecvBufferE[MYSUB + ly];
    }
  }
}
...
static void PrepareExt(N_Vector u, UserData data)
{
  ...
  if (data->isuby == 0)
  {
    for (lx = 0; lx < MXSUB; lx++) { data->c1ext[1 + lx] = c1data[MXSUB + lx]; }
    for (lx = 0; lx < MXSUB; lx++) { data->c2ext[1 + lx] = c2data[MXSUB + lx]; }
  }
  ...
  if (data->isuby == NPEY - 1)
  {
    for (lx = 0; lx < MXSUB; lx++)
    {
      data->c1ext[(MYSUB + 1) * (MXSUB + 2) + 1 + lx] =
        c1data[(MYSUB - 2) * MXSUB + lx];
    }
    for (lx = 0; lx < MXSUB; lx++)
    {
      data->c2ext[(MYSUB + 1) * (MXSUB + 2) + 1 + lx] =
        c2data[(MYSUB - 2) * MXSUB + lx];
    }
  }
...
static void fcalc(sunrealtype t, N_Vector udot, UserData data)
{
  ...
  for (ly = 0; ly < MYSUB; ly++)
  {
    ...
    for (lx = 0; lx < MXSUB; lx++)
    {
      ...
      hord1  = (data->hdco) * (c1rt - SUN_RCONST(2.0) * c1 + c1lt);
      hord2  = (data->hdco) * (c2rt - SUN_RCONST(2.0) * c2 + c2lt);
