           ...
           sunbooleantype projerr, N_Vector yref)
{
  ...
  {
    sprintf(outname,
            "cv_pendulum_dns_rtol_%03.2" ESYM "_atol_%03.2" ESYM "_proj.txt",
            rtol, atol);
  }
  else
  {
    sprintf(outname, "cv_pendulum_dns_rtol_%03.2" ESYM "_atol_%03.2" ESYM ".txt",
            rtol, atol);
  }
  FID = fopen(outname, "w");
...
int RefSol(sunrealtype tf, N_Vector yref, int nout)
{
  ...
  /* Open output file */
  FID = fopen("cv_pendulum_dns_ref.txt", "w");
