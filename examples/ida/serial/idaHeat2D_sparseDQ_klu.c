...
static int JacPattern(SUNMatrix JJ);
...
int main(void)
{
  ...
  /* Use IDA's internal sparse difference-quotient Jacobian approximation. */
  retval = IDASetJacFn(mem, NULL);
...
/* Jacobian matrix sparsity pattern setup for MGRID>=4 */
static int JacPattern(SUNMatrix JJ)
{
  int i, j, repeat = 0;
  ...
  if (SUNSparseMatrix_SparseType(JJ) != SUN_CSC_MAT) { return (-1); }
