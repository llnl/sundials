...
# -----------------------------------------------------------------
# 1D heat equation example using sundials4py and JAX on CPU or CUDA.
# -----------------------------------------------------------------
...
def main():
    ...
    if array_device == "cuda":
        device_result = np.asarray(
            sun.N_VGetJaxArray(y, device="cpu", copy_from="device")
        )
    else:
