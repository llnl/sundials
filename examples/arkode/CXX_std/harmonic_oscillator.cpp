...
#include <nvector/nvector_stdvector.hpp>
#include <sundials/sundials_nvector_base.hpp>

using sundials::StdVector;
...
{
  const auto* yv = StdVector::extractConst(y);
  auto* ydotv    = StdVector::extract(ydot);

  /* Access the underlying std::vector data directly. */
  const auto* yd = yv->data();
  auto* fd       = ydotv->data();
...
int main()
{
  ...
  StdVector y(neq, sunctx);
  y.data()[0] = 1.0; /* y1(0) = cos(0) = 1 */
  y.data()[1] = 0.0; /* y2(0) = sin(0) = 0 */
  ...
  std::printf("  %8.4f  %14.10f  %14.10f  %14.10f  %14.10f  %10.2e\n", t0,
              y.data()[0], y.data()[1], std::cos(t0), -std::sin(t0),
              std::hypot(y.data()[0] - std::cos(t0), y.data()[1] + std::sin(t0)));
  ...
  while (tout <= tf + 0.5 * dt)
  {
    ...
    sunrealtype error    = std::hypot(y.data()[0] - y1_exact,
                                      y.data()[1] - y2_exact);

    std::printf("  %8.4f  %14.10f  %14.10f  %14.10f  %14.10f  %10.2e\n", tret,
                y.data()[0], y.data()[1], y1_exact, y2_exact, error);
