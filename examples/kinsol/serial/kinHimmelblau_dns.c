 ...
 * Test problem 1 from Section 14.1, Chapter 14: Himmelblau function
 *
* The goal of this example is to find all stationary points of the Himmelblau
* function (Reklaitis and Ragsdell, 1983).
 *    4x_{1}^{3}+4x_{1}x_{2}+2x_2^{2}-42x_2-14 = 0
 *    4x_{2}^{3}+2x_{1}^{2}+x_{2}+4x_{1}x_{2}-26x_2-22 = 0
 * such that
 *    -5 <= x1 <= 5
 *    -5 <= x2 <= 5
 *
 ...
 *    L1 <= 0    L2 <= 0
 *
 * The Himmelblau function test problem has nine known solutions.
 * The nonlinear system is solved by KINSOL using different
 * combinations of globalization and Jacobian update strategies
 * and with different initial guesses (leading to one or the others
...
int main(void)
{
  ...
  data->lb[0] = SUN_RCONST(-5.0);
  data->ub[0] = SUN_RCONST(5.0);
  data->lb[1] = SUN_RCONST(-5.0);
  data->ub[1] = SUN_RCONST(5.0);
  ...
  Ith(c, 3) = SUN_RCONST(1.0);  /* l1 = x1 - x1_min >= 0 */
  Ith(c, 4) = SUN_RCONST(-1.0); /* L1 = x1 - x1_max <= 0 */
  Ith(c, 5) = SUN_RCONST(1.0);  /* l2 = x2 - x2_min >= 0 */
  Ith(c, 6) = SUN_RCONST(-1.0); /* L2 = x2 - x22_min <= 0 */
...
static int func(N_Vector u, N_Vector f, void* user_data)
{
  ...
  fdata[0] = SUN_RCONST(4.0)*SUNRpowerI(x1,3)+SUN_RCONST(4.0)*x1*x2+
             SUN_RCONST(2.0)*SUNRpowerI(x2,2)-SUN_RCONST(42.0)*x1-SUN_RCONST(14.0);
  fdata[1] = SUN_RCONST(4.0)*SUNRpowerI(x2,3)+SUN_RCONST(4.0)*x1*x2+
             SUN_RCONST(2.0)*SUNRpowerI(x1,2)-SUN_RCONST(26.0)*x2-SUN_RCONST(22.0);
  fdata[2] = l1 - x1 + lb[0];
  fdata[3] = L1 - x1 + ub[0];
  fdata[4] = l2 - x2 + lb[1];
  fdata[5] = L2 - x2 + ub[1];
