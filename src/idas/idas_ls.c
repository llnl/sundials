...
/*---------------------------------------------------------------
  ...
  the DAE system Jacobian F_y + c_j*F_y', retaining the input CSC
  SUNMatrix sparsity pattern.

  Rather than perturbing one solution component at a time (as the dense
  routine does, at a cost of N residual evaluations), the columns of the
  ...
  one residual evaluation per group, which for a banded or otherwise
                     ...
                     N_Vector tmp2, N_Vector tmp3)
{
  ...
  if ((idals_mem->sparseDQgroups == NULL) || (idals_mem->sparseDQM != M) ||
  ...
  for (group = 0; group < ngroups; group++)
  {
    ...
    for (j = 0; j < N; j++)
    {
      ...
      /* Set increment using the same formula as the dense/band DQ routines.
         This computation is duplicated in the restore loop below -- the two
    ...
    /* Restore ytemp/yptemp, then form and load difference quotients. Within
       this group every row touched by column j was claimed by column j
       alone, so rtemp_data[row] - r_data[row] isolates column j's
       contribution. */
    for (j = 0; j < N; j++)
    {
      if (groups[j] != group) { continue; }
      ...
      /* Recompute inc exactly as in the perturbation loop above. It depends
         only on yj, ypj and ewtj, all of which have just been restored to
         their original values, so this reproduces the increment that was
         actually applied. The increment is recomputed rather than cached
         because tmp3 is already in use as yptemp. */
      inc = SUNMAX(srur * SUNMAX(SUNRabs(yj), SUNRabs(IDA_mem->ida_hh * ypj)),
                   ONE / ewtj);
      if (IDA_mem->ida_hh * ypj < ZERO) { inc = -inc; }
      inc = (yj + inc) - yj;
  ...
  if (idals_mem->J)
  {
    ...
    if (SUNLinSolGetType(idals_mem->LS) == SUNLINEARSOLVER_DIRECT)
    {
      /* The internal sparse DQ Jacobian is skipped here: SUNMatZero_Sparse
         ...
         sparsity pattern that idaLsSparseDQJac fills into. Zeroing is
         unnecessary in any case, since that routine overwrites every stored
         value. Unlike CVODE and ARKODE, IDAS needs no savedJ copy to recover
