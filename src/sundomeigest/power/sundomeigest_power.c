                                             ...
                                             sunrealtype* lambdaI)
{
  ...
  /* the 2x2 projection used to recover a complex pair from a real iteration
     is only needed when the scalar type is real */
#if defined(SUNDIALS_SCALAR_TYPE_REAL)
  ...
  *num_iters         = 0;
  ...
  if (is_complex)
  {
    retval = sundomeigestimator_complex_dom_eigs_from_PI(DEE,
                                                         SUN_REAL(newlambdaR),
                                                         normAv, v_prev, V,
                                                         lambdaR, lambdaI);
    if (retval != 0) { return SUN_ERR_USER_FCN_FAIL; }
