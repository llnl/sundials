...
int mriStep_TakeStepMRIGARK(ARKodeMem ark_mem, sunrealtype* dsmPtr, int* nflagPtr)
{
  ...
  *dsmPtr   = ZERO;
  ...
  /* Loop over all but the final stage (and potentially the embedding stage) */
  ...
  if (do_embedding)
  {
    ...
    /* Temporarily swap ark_mem->ycur and ark_mem->lte pointers, copying
       ...
       that during this embedding "stage":
         ...
         - ark_mem->lte will hold the embedded solution vector. */
    N_VScale(ONE, ark_mem->ycur, ark_mem->lte);
    tmp           = ark_mem->ycur;
    ark_mem->ycur = ark_mem->lte;
    ark_mem->lte  = tmp;
    ...
    /* Swap back ark_mem->ycur with ark_mem->lte, and reset the inner integrator */
    tmp           = ark_mem->ycur;
    ark_mem->ycur = ark_mem->lte;
    ark_mem->lte  = tmp;
  ...
  *dsmPtr   = ZERO;
  ...
  if (adapt_type == SUN_ADAPTCONTROLLER_MRI_H_TOL)
  {
    ...
    if (retval != ARK_SUCCESS)
    {
      arkProcessError(ark_mem, ARK_INNERSTEP_FAIL, __LINE__, __func__, __FILE__,
                      "Unable to reset the inner stepper error estimate");
      (void)SUNVecStack_Push(ark_mem->temp_vec_stack, &ytilde);
      return (ARK_INNERSTEP_FAIL);
    }
    retval = mriStepInnerStepper_SetRTol(step_mem->stepper,
                                         step_mem->inner_rtol_factor *
