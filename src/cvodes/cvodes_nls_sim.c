...
int CVodeSetNonlinearSolverSensSim(void* cvode_mem, SUNNonlinearSolver NLS)
{
  ...
  retval = SUNNonlinSolSetGetUpdateNormFn(cv_mem->NLSsim,
                                          cvNlsGetUpdateNormSensSim, cvode_mem);
  if (retval != CV_SUCCESS)
  {
    cvProcessError(cv_mem, CV_ILL_INPUT, __LINE__, __func__, __FILE__,
  ...
  retval = SUNNonlinSolSetGetConvRateFn(cv_mem->NLSsim, cvNlsGetConvRateSensSim,
                                        cvode_mem);
  if (retval != CV_SUCCESS)
  {
    cvProcessError(cv_mem, CV_ILL_INPUT, __LINE__, __func__, __FILE__,
  ...
  */
  if (m > 0)
  {
    cv_mem->cv_crate = SUNMAX(CRDOWN * cv_mem->cv_crate,
                              cv_mem->cv_delnrmS / cv_mem->cv_delp);
  }
  dcon = cv_mem->cv_delnrmS * SUNMIN(ONE, cv_mem->cv_crate) / tol;
  ...
  if (dcon <= ONE)
  {
    if (m == 0)
    {
      cv_mem->cv_acnrm = (cv_mem->cv_errconS) ? cv_mem->cv_delnrmS
                                              : cv_mem->cv_delnrm;
    }
    else
    {
  ...
  if ((m >= 1) && (cv_mem->cv_delnrmS > RDIV * cv_mem->cv_delp))
  {
    return (SUN_NLS_CONV_RECVR);
  }

  /* Save norm of correction and loop again */
  cv_mem->cv_delp = cv_mem->cv_delnrmS;
