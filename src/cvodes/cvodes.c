...
int CVodeQuadSVtolerances(void* cvode_mem, sunrealtype reltolQ, N_Vector abstolQ)
{
  ...
  N_VCopy(abstolQ, cv_mem->cv_VabstolQ);
...
static int cvStep(CVodeMem cv_mem)
{
  ...
  int ncfS  = 0; /* sensitivity corrector failures           */
  int nefS  = 0; /* sensitivity error test fails             */
  int nefQ  = 0; /* quadrature error test fails              */
  int nefQS = 0; /* quadrature sensitivity error test fails  */
...
static void cvCompleteStep(CVodeMem cv_mem)
{
  ...
  if ((cv_mem->cv_qwait == 1) && (cv_mem->cv_q != cv_mem->cv_qmax))
  {
    ...
    if (cv_mem->cv_quadr)
    {
      N_VCopy(cv_mem->cv_acorQ, cv_mem->cv_znQ[cv_mem->cv_qmax]);
    }
...
static void cvPrepareNextStep(CVodeMem cv_mem, sunrealtype dsm)
{
  ...
  else if (cv_mem->cv_skipadapttstop && cv_mem->cv_tstoplimited)
  {
    ...
    cv_mem->cv_qprime = cv_mem->cv_qsave;
    cv_mem->cv_hprime = cv_mem->cv_hsave;
    cv_mem->cv_eta    = cv_mem->cv_hsave / cv_mem->cv_h;
  }
  else
  {
...
static int cvRootfind(CVodeMem cv_mem)
{
  ...
  { /* Looping point */
    ...
    for (i = 0; i < cv_mem->cv_nrtfn; i++)
    {
      ...
      {
        if ((SUNRdifferentsign(cv_mem->cv_glo[i], cv_mem->cv_grout[i])) &&
            (cv_mem->cv_rootdir[i] * cv_mem->cv_glo[i] <= ZERO))
        {
