...
%===================================================================================
\subsection{A serial dense example: kinHimmelblau\_dns}\label{ss:kinHimmelblau_dns}
As an initial illustration of the use of the {\kinsol} package for the
solution of nonlinear systems, we give a sample program called \id{kinHimmelblau_dns.c}.
It uses the {\kinsol} dense linear solver module {\sunlinsoldense}
and the {\nvecs} module (which provides a serial implementation of {\nvector})
for the solution of the Himmelblau function test problem~\cite{FlPa:99}.
...
\begin{equation}
  \begin{split}
    & 0 = 4x_{1}^{3}+4x_{1}x_{2}+2x_2^{2}-42x_2-14 \\
    & 0 = 4x_{2}^{3}+2x_{1}^{2}+x_{2}+4x_{1}x_{2}-26x_2-22  \\
    &\text{subject to } \\
    &\qquad x_{1\min} = -5 \le x_1 \le 5 = x_{1\max} \\
    &\qquad x_{2\min} = -5 \le x_2 \le 5 = x_{2\max} \, .
  \end{split}
\end{equation}
...
\begin{equation*}
  ...
  \end{split}
\end{equation*}
The Himmelblau function problem has nine known solutions. We solve it with
