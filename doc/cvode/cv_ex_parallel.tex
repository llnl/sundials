...
\subsection{A nonstiff example: cv_adv_diff\_non\_p}\label{ss:cv_adv_diff_p}
...
\dot{u}_i=\frac{u_{i+1}-2u_{i}+u_{i-1}}{(\Delta x)^{2}}
  + 0.5 \frac{u_{i+1}-u_{i-1}}{2(\Delta x)} ~. \label{cv_adv_diff_pode}
\end{equation}
...
the computation of the right hand side of Eq. (\ref{cv_adv_diff_pode}) requires
...
The \id{cv_adv_diff\_non\_p.c} file begins with \id{\#include} declarations for
...
The \id{f} function is an implementation of Eq. (\ref{cv_adv_diff_pode}), but preceded
...
The \id{cv_adv_diff\_non\_p.c} file includes a routine \id{check\_flag} that checks the
return values from calls in \id{main}.  This routine was written to be used
by any parallel {\sundials} application.

The output below is for \id{cv_adv_diff\_non\_p} with \id{MX} = 10 and four processors.
...
%%
\includeOutput{cv_adv_diff\_non\_p}{../../examples/cvode/cv_adv_diff/c-mpi/cv_adv_diff_non_p.out}
%%

%-------------------------------------------------------------------------------

\subsection{A user preconditioner example: cv_diurnal\_kry\_p}\label{ss:cv_diurnal_p}
...
the system PDEs given above for the \id{cv_diurnal\_kry} example.
As before, we discretize the PDE system with central differencing, to
obtain an ODE system $\dot{u} = f(t,u)$ representing (\ref{cv_diurnalpde}).
...
\id{cv_adv_diff\_non\_p}, and to provide a template for a stiff ODE system
...
The organization of the \id{cv_diurnal\_kry\_p} program deserves some comments. The
...
\id{uext} only.  Most other features of \id{cv_diurnal\_kry\_p.c} are the same as
in \id{cv_diurnal\_kry.c}, except for extra logic involved with distributed
vectors.

The following is a sample output from \id{cv_diurnal\_kry\_p}, for four processors
(in a $2 \times 2$ array) with a $5 \times 5$ subgrid on each.
The output will vary slightly if the number of processors is changed.

%%
\includeOutput{cv_diurnal\_kry\_p}{../../examples/cvode/cv_diurnal/c-mpi/cv_diurnal_kry_p.out}
%%

%-------------------------------------------------------------------------------

\subsection{A CVBBDPRE preconditioner example: cv_diurnal\_kry\_bbd\_p}\label{ss:cv_diurnal_bbd_p}

In this example, \id{cv_diurnal\_kry\_bbd\_p}, we solve the same
problem as in \id{cv_diurnal\_kry\_p}
...
As in \id{cv_diurnal\_kry\_p.c}, the \id{f} routine in \id{cv_diurnal\_kry\_bbd\_p.c} simply calls a
...
In \id{cv_diurnal\_kry\_bbd\_p.c}, the problem is solved twice --- first with preconditioning
...
Sample output from \id{cv_diurnal\_kry\_bbd\_p} follows, again using $5 \times 5$ subgrids
...
\id{nli/nni}, is very close to that of \id{cv_diurnal\_kry\_p} when preconditioning is on
the left, but slightly poorer when it is on the right.

%%
\includeOutput{cv_diurnal\_kry\_bbd\_p}{../../examples/cvode/cv_diurnal/c-mpi/cv_diurnal_kry_bbd_p.out}
%%
