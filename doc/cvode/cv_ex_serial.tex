...
\subsection{A dense example: cv_roberts\_dns}\label{ss:cv_roberts}

As an initial illustration of the use of the {\cvode} package for the
integration of IVP ODEs, we give a sample program called \id{cv_roberts\_dns.c}.
...
The output generated by \id{cv_roberts\_dns} is shown below.  It shows the output
...
%%
\includeOutput{cv_roberts\_dns}{../../examples/cvode/cv_roberts/c/cv_roberts_dns.out}
%%

%-------------------------------------------------------------------------------

\subsection{A banded example: cv_adv_diff\_bnd}\label{ss:cv_adv_diff}

The example program \id{cv_adv_diff\_bnd.c} solves the semi-discretized form of
...
The \id{cv_adv_diff\_bnd.c} program includes files
...
Following the \id{main} program in the \id{cv_adv_diff\_bnd.c} file are definitions of
...
only from within the \id{cv_adv_diff\_bnd.c} file.
...
The output generated by \id{cv_adv_diff\_bnd} is shown below.

%%
\includeOutput{cv_adv_diff\_bnd}{../../examples/cvode/cv_adv_diff/c/cv_adv_diff_bnd.out}
%%

%-------------------------------------------------------------------------------

\subsection{A Krylov example: cv_diurnal\_kry}\label{ss:cv_diurnal}
...
\begin{equation}\label{cv_diurnalpde}
...
\begin{equation}\label{e:cv_diurnal:r}
\begin{split}
...
\begin{equation} \label{cv_diurnalic}
  \begin{split}
  ...
  \end{split} 
\end{equation}
For this example, the equations (\ref{cv_diurnalpde}) are discretized spatially
...
linear solver data structures, \id{cv_diurnal\_kry.c} uses underling small
...
In addition to the functions called by {\cvode}, \id{cv_diurnal\_kry.c} includes
...
\id{PrintOutput} to retrieve and print selected solution values and
statistics; \id{PrintFinalStats} to print statistics; and \id{check\_flag}
to check return values for error conditions.

The output generated by \id{cv_diurnal\_kry.c} is shown below.  Note that the
...
Jacobian reuse scheme.

%%
\includeOutput{cv_diurnal\_dns}{../../examples/cvode/cv_diurnal/c/cv_diurnal_kry.out}
%%
