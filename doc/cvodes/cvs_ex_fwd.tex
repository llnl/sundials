...
This example is a modification of the chemical kinetics example \id{cv_roberts\_dns}
...
These explanations, together with those given for the code \id{cv_roberts\_dns}
...
defined in \id{cv_roberts\_dns}, we define the number of model parameters, \id{NP} ($=3$),
...
%%
The next block of code is identical to that in \id{cv_roberts\_dns.c} (see \cite{cvode_ex})
...
\id{N\_VCloneVectorArray\_Serial}, and initializes all sensitivity variables to $0.0$.
...
\id{Jac} (for the system Jacobian) are identical to those in \id{cv_roberts\_dns.c},
...
\id{cv_roberts\_dns\_uw.c}.  The user-supplied function \id{fS} computes the
...
a test problem (derived from \id{cv_diurnal\_kry\_p}) that solves the
...
\id{cv_diurnal\_kry\_p} described in \cite{cvode_ex}. The only difference is in the
